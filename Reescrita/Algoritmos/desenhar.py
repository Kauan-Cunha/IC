...
import Grafo.Grafo as g  # Certifique-se de importar sua classe corretamente

def desenhar_grafo(grafo: g.Grafo, layout='circular'):
    """
    Desenha um grafo direcionado e ponderado a partir de um objeto da classe Grafo.
    ...
    Args:
        grafo (g.Grafo): Objeto da classe Grafo.
        layout (str): O algoritmo de layout a ser usado ('circular', 'spring', etc.).
    """
    # 1. Usar MultiDiGraph para aceitar arestas paralelas
    G = nx.MultiDiGraph()

    # Adicionar todos os nós (inclusive os isolados)
    for i in range(grafo.num_vertices):
        G.add_node(i)

    # Adicionar arestas a partir do objeto Grafo
    # grafo.obter_arestas() retorna tuplas (origem, destino, peso)
    for u, v, peso in grafo.obter_arestas():
    ...
    # G.edges(keys=True) itera sobre todas as arestas do MultiDiGraph
    ...
    # Agora desenhamos, calculando o rad (curvatura)
    for pair, keys in edge_rads.items():
        ...
        for i, key in enumerate(keys):
            ...
            # Se só tiver uma aresta indo, mas existir uma voltando, curvamos para não sobrepor.
    ...
    # --- 5. Lógica para Rótulos Agregados ---
    
    unique_edges_uv = set(G.edges(keys=False))
    aggregated_labels = {}
    for u, v in unique_edges_uv:
        key_data_dict = G.get_edge_data(u, v)
        # Formata o peso como string; se for float, pode formatar (ex: f"{peso:.1f}")
    ...
    plt.title("Grafo Ponderado (Classe Grafo)")
    plt.axis('off')
    plt.show()